...
13. **Operation Identity:** Externally supplied scheduled operations must use unique positive IDs. Invalid IDs are rejected before queue partitioning or worker startup.
14. **JavaScript Seed Precision:** TypeScript and WebAssembly callers accept seeds from `0` through `Number.MAX_SAFE_INTEGER`. Values outside that range are rejected so the serialized seed cannot silently change through JavaScript number rounding. Native and Python interfaces retain the full unsigned 64-bit range.
